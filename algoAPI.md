...
| 術語 | 說明 |
|------|------|
| Carrier (載體) | 用來藏秘密的圖片，使用者上傳 |
| Secret (秘密) | 要被藏起的圖案，使用者上傳 |
| Background Color | 秘密圖片的底色（黑/白），用於計算目標值 |
| Target Size | 目標縮圖尺寸（由平台決定） |
| Thumbnail | 經過下採樣後的結果 |
...
const PLATFORM_CONFIGS = {
  discord: { name: 'Discord', maxWidth: 500, maxHeight: 500, kernel: 'bicubic', quality: 0.8 },
  line:    { name: 'LINE',    maxWidth: 1200, maxHeight: 1200, kernel: 'bilinear', quality: 0.85 },
  twitter: { name: 'X (Twitter)', maxWidth: 1200, maxHeight: 675, kernel: 'bicubic', quality: 0.9 },
  custom:  { name: 'Custom',  maxWidth: 1920, maxHeight: 1080, kernel: 'lanczos', quality: 1.0 },
};
```
