...
import { bruteForceXProcess } from '@lib/bruteForceXProcessor';
import type { XProcessResult } from '@lib/bruteForceXProcessor';
...
export function useBruteForceProcessor() {
  ...
  const processImage = useCallback(async (file: File) => {
    ...
    try {
      const result = await bruteForceXProcess(file);
