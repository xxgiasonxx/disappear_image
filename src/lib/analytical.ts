...
/** Zero-mean pair around `base` with no clipping and dynamic range. */
function zeroMean(base: number, strength: number, isEven: boolean): number {
  // Full dynamic-range extreme values that average to `base` without clipping:
  // a + b = 2 * base, a in [0, 255], b in [0, 255]
  // a = max(0, 2*base - 255),  b = min(255, 2*base)
  const fullDark  = Math.max(0, 2 * base - 255);
  const fullLight = Math.min(255, 2 * base);

  // Interpolate according to strength (0 => no dithering, 1 => full range)
...
export function hideImageAnalytical(
  ...
  for (let ty = 0; ty < targetHeight; ty++) {
    for (let tx = 0; tx < targetWidth; tx++) {
      ...
      // 1) Compute the average of NON-SECRET pixels (true background)
      ...
      // 2) Check if this block touches any secret pixel
      let hasSecret = false;
      ...
      // 3) Dither ONLY the secret pixels, using block average as zero-mean base
      for (let y = sy0; y < sy1; y++) {
        for (let x = sx0; x < sx1; x++) {
          if (!isSecretPixel(x, y)) continue;
